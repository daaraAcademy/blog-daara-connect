<template>
  <footer class="relative z-10 border-t border-gray-3 py-8">
    <div class="mx-auto flex max-w-[1170px] flex-col gap-4 px-4 sm:px-8 xl:px-0 lg:flex-row lg:items-center lg:justify-between lg:gap-0">
      <p class="text-custom-sm">&copy; {{ new Date().getFullYear() }} Daara Connect. Tous droits réservés.</p>

      <ul class="flex flex-wrap items-center gap-2.5">
        <li>
          <NuxtLink to="/confidentialite" class="group flex text-custom-sm leading-none duration-200 ease-in hover:text-dark">
            <span class="bg-linear-to-r from-dark to-dark bg-left-bottom bg-no-repeat transition-[background-size] duration-500 hover:bg-[length:100%_3px] group-hover:bg-[length:100%_1px]">
              Confidentialité
            </span>
          </NuxtLink>
        </li>
        <li><span class="flex h-[3px] w-[3px] rounded-full bg-dark-2"></span></li>
        <li>
          <NuxtLink to="/confidentialite" class="group flex text-custom-sm leading-none duration-200 ease-in hover:text-dark">
            <span class="bg-linear-to-r from-dark to-dark bg-left-bottom bg-no-repeat transition-[background-size] duration-500 hover:bg-[length:100%_3px] group-hover:bg-[length:100%_1px]">
              Conditions
        ...
        <li><span class="flex h-[3px] w-[3px] rounded-full bg-dark-2"></span></li>
        <li>
          <NuxtLink to="/inscription" class="group flex text-custom-sm leading-none duration-200 ease-in hover:text-dark">
            <span class="bg-linear-to-r from-dark to-dark bg-left-bottom bg-no-repeat transition-[background-size] duration-500 hover:bg-[length:100%_3px] group-hover:bg-[length:100%_1px]">
              Contact
            </span>
          </NuxtLink>
      ...
      <div class="flex items-center gap-3">
        <p class="text-custom-sm font-medium text-dark">Suivez-nous :</p>
        <div class="flex items-center gap-1.5">
          <a
            aria-label="Facebook"
            href="https://facebook.com"
            class="flex h-7.5 w-7.5 items-center justify-center rounded-full transition-all duration-300 ease-linear hover:bg-gray-2 hover:text-dark"
            target="_blank"
            rel="noopener"
          >
